...
class CBoard:
    ...
    def listMoveMa(self, id):
        ...
        for d in range(8):
            ...
            xx = x + ax[d]
            yy = y + ay[d]
            if 0 <= xx <= 8 and 0 <= yy <= 9 and self.board[xx][yy] == -1:
    ...
    def listMoveXiang(self, id):
        ...
        for d in range(4):
            ...
            yy = y + ay[d]
            if 0 <= xx <= 8 and 0 <= yy <= 9 and self.board[xx][yy] == -1:
                if 0 <= nx <= 8 and 0 <= ny <= 9 and not self.isOutRiver(id, ny):
                    self.pushList(id, nx, ny)
    ...
    def listMoveJiang(self, id):
        ...
        for d in range(st, ed + 1, step):
            ny = y + d
            if 0 <= ny <= 9 and self.board[x][ny] != -1:
                if self.pad[self.board[x][ny]][0] == 'Jiang':
                    self.pushList(id, x, ny)
                else:
                    break
